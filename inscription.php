		...
		<section>
	    	...
	    	if(isset($_POST['Inscrire'])){
	    		...
	    		if (isset($_POST['nom']) && !empty($_POST['nom']) && isset($_POST['prenom']) && !empty($_POST['prenom']) && isset($_POST['mdp']) && !empty($_POST['mdp']) && isset($_POST['confirm_mdp']) && !empty($_POST['confirm_mdp']) && isset($_POST['email']) && !empty($_POST['email']) ){
					...
					//IL FAUT COMPARER SI LE NOM ET LE PRENOM INDIQUES EXISTES DANS LA TABLE MEMBRESLIGUES
	    					$req = $bdd->query('SELECT * FROM MEMBRESLIGUES');
	    			...
	    			if (strlen($mdp) >= 5 || strlen($confirm_mdp) >= 5 ){
	    				if($mdp==$confirm_mdp){
	    					$mdp = sha1($mdp);
	    					//var_dump($mdp); exit;
	    					$req5 = $bdd->query("SELECT COUNT(*) AS nbutils FROM utilisateurs");
	    					$occurence = $req5->fetch();
							...
							if($verite == true)
							{
						    	...
						    	}else $erreur = "<strong>Vous n'êtes pas un membre des ligues</strong><br/>"; 
